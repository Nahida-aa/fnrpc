//! Rust client-side BigInt envelope codec for fnrpc.
//!
//! On the wire, `BigInt`-style values cannot be represented in JSON, so they
//! are transmitted as strings wrapped in a metadata envelope (`{ json, meta }`).
//! This module reverses that transformation so a Rust consumer receives the
//! original numeric values.
//!
//! The server performs an equivalent, schema-driven conversion internally
//! (see `fnrpc::serializer::decode_bigint_by_schema`); this function is the
//! client-side analogue and a reusable reference implementation of the
//! envelope protocol.

use serde_json::Value;

/// BIGINT type ID — must match `BIGINT` constant in the TS serializer.

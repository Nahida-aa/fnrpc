...
use fnrpc_web::{RpcWebConfig, handle};
...
impl RpcFn<()> for Noop {
    ...
    fn exec<'a>(_ctx: &'a (), _input: ()) -> impl Future<Output = Result<(), RpcErr>> + Send + 'a {
        async move { Ok(()) }
    }
}
...
#[fnrpc::rpc_query]
async fn echo_get(input: String) -> String {
    input
}
#[fnrpc::rpc_mutate]
...
pub(crate) async fn bench(n: usize) {
    ...
    let s = HeapStats::get();
    eprintln!(
        "fnrpc-web/noop_json: {:>8}B, {:>6} blks  ({:>6.1}B, {:>5.1}blks/op)",
        s.total_bytes,
        s.total_blocks,
    ...
    let _ = std::fs::copy(
        ...
        "./benches/target/dhat-fnrpc-web-noop-json.json",
    );
    ...
    let _ = std::fs::copy(
        "./benches/target/dhat-heap.json",
        "./benches/target/dhat-fnrpc-web-noop-raw.json",
    );
    ...
    let _ = std::fs::copy(
        "./benches/target/dhat-heap.json",
        "./benches/target/dhat-fnrpc-web-echo-post.json",
    );
    ...
    let s = HeapStats::get();
    eprintln!(
        "fnrpc-web/not_found: {:>8}B, {:>6} blks  ({:>6.1}B, {:>5.1}blks/op)",
        s.total_bytes,
        s.total_blocks,
        s.total_bytes as f64 / n as f64,
        s.total_blocks as f64 / n as f64
    ...
    let _ = std::fs::copy(
        "./benches/target/dhat-heap.json",
        "./benches/target/dhat-fnrpc-web-notfound.json",
    );
}
